...
def make_agent():
    ...
    settings = cfg(
        ...
        worker_scale_warmup_steps=2,
        worker_scale_full_steps=5,
    ...
    for p in agent._frozen_hierarchical_options.parameters():
...
def test_hierarchical_auxiliary_loss_is_finite_and_routes_gradients():
    ...
    loss.backward()
    assert agent.hierarchical_options.manager[0].weight.grad is not None
