...
def main() -> None:
    ...
    if dreamer.index("losses.pop(legacy_key, None)") > dreamer.index(
        "total_loss = sum([v * self._loss_scales[k]"
    ):
        fail("legacy behavior losses are disabled after total-loss construction")
