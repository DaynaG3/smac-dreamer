...
def patch_checkpoint_resume_ast(text: str) -> str:
    """Patch checkpoint loading without depending on source formatting."""
    tree = ast.parse(text)
    ...
    for node in ast.walk(tree):
        ...
        if "agent_state_dict" in source and "ckpt" in source:
            load_calls.append(node)

    if len(load_calls) != 1:
        die(
            "checkpoint migration: expected exactly one "
            "agent.load_state_dict checkpoint call, found "
            f"{len(load_calls)}"
        )
    ...
    load_stmt = enclosing_statement(load_calls[0])
    if load_stmt is None:
        die("checkpoint migration: could not locate load statement")
    ...
    for node in ast.walk(tree):
        ...
        source = ast.get_source_segment(text, node) or ""
        if (
            "agent_training_state" in source
            and "load_training_state_dict" in source
            and node.lineno > load_stmt.lineno
        ):
            training_if_matches.append(node)

    if len(training_if_matches) != 1:
        die(
            "checkpoint migration: expected exactly one optimizer-state "
            f"restore block, found {len(training_if_matches)}"
        )

    training_if = training_if_matches[0]

    load_parent = parents.get(load_stmt)
    training_parent = parents.get(training_if)

    if load_parent is not training_parent:
        die(
            "checkpoint migration: model and optimizer restore statements "
            "are not in the same enclosing block"
        )

    sibling_body = getattr(load_parent, "body", None)
    if not isinstance(sibling_body, list):
        die("checkpoint migration: enclosing block has no statement body")

    try:
        load_index = sibling_body.index(load_stmt)
        training_index = sibling_body.index(training_if)
    except ValueError:
        die("checkpoint migration: restore statements not found in parent body")

    if training_index != load_index + 1:
        die(
            "checkpoint migration: unexpected statements occur between "
            "model loading and optimizer restoration"
        )

    lines = text.splitlines(keepends=True)
    start_line = load_stmt.lineno - 1
    end_line = training_if.end_lineno
    ...
    replacement = (
        f'{indent}tactical_load = agent.load_tactical_compatible_state_dict(\n'
        f'{i1}ckpt["agent_state_dict"],\n'
        f'{i1}checkpoint_metadata=ckpt.get(\n'
        f'{i2}"tactical_mixture_metadata"\n'
        f'{i1}),\n'
        f'{indent})\n'
        f'{indent}can_restore_training = (\n'
        f'{i1}not tactical_load.get("migrated_legacy", False)\n'
        f'{i1}and ckpt.get("agent_training_state")\n'
        f'{i1}and hasattr(agent, "load_training_state_dict")\n'
        f'{indent})\n'
        f'{indent}if can_restore_training:\n'
        f'{i1}agent.load_training_state_dict(\n'
        f'{i2}ckpt["agent_training_state"]\n'
        f'{i1})\n'
        f'{i1}print(\n'
        f'{i2}f" [resume] restored tactical model + optimizer state from {{args.resume}}"\n'
        f'{i1})\n'
        f'{indent}elif tactical_load.get("migrated_legacy", False):\n'
        f'{i1}print(\n'
        f'{i2}" [resume] migrated legacy weights; tactical modules are zero-init"\n'
        f'{i1})\n'
        f'{i1}print(\n'
        f'{i2}" [resume] optimizer/scheduler/return EMA start fresh"\n'
        f'{i1})\n'
        f'{indent}else:\n'
        f'{i1}print(\n'
        f'{i2}f" [resume] restored model weights only from {{args.resume}}"\n'
        f'{i1})\n'
    )
...
def patch_training_script(text: str) -> str:
    ...
    if " tactical : enabled=" not in text:
        import ast as _ast

        try:
            runner_tree = _ast.parse(text)
        except SyntaxError as exc:
            die(
                "startup print: runner failed AST parsing before patch: "
                f"{exc}"
            )

        replay_print_nodes = []

        for node in _ast.walk(runner_tree):
            ...
            source = _ast.get_source_segment(text, node) or ""

            if (
                "config.buffer.storage_backend" in source
                and "config.buffer.max_size" in source
                and "config.buffer.scratch_dir" in source
            ):
                replay_print_nodes.append(node)

        if len(replay_print_nodes) != 1:
            die(
                "startup print: expected exactly one distinct "
                "replay-summary print statement, found "
                f"{len(replay_print_nodes)}"
            )

        replay_print = replay_print_nodes[0]
        lines = text.splitlines(keepends=True)

        insertion_index = replay_print.lineno - 1
        lines.insert(insertion_index, tactical_startup_print)
    ...
    text = patch_checkpoint_resume_ast(text)
    text = replace_once(
        text,
