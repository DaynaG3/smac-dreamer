...
class HierarchicalOptionSettings:
    ...
    commitment_full_steps: int = 300_000
    ...
    manager_unimix_decay_steps: int = 300_000
    ...
    worker_pg_warmup_steps: int = 100_000
    worker_pg_full_steps: int = 300_000
    ...
    # Keep the source JEPA representation fixed for this conservative 2M
    # hierarchy phase. A later experiment may explicitly opt into adaptation.
    ...
    # from 5-8 to 7-10; this extends credit assignment without jumping directly
    # to 15-step rollouts before the JEPA model is validated at that horizon.
    imag_horizon_initial_max: int = 8
    imag_horizon_final_max: int = 10
    imag_horizon_window: int = 4
    imag_horizon_ramp_steps: int = 500_000
...
class HierarchicalOptionsPolicy(nn.Module):
    ...
    def reset_parameters(self) -> None:
        ...
        # Initialise the smooth bounded learned branch to the fixed hazard. The
        # bound remains differentiable, unlike a hard clamp.
        term_floor = 0.5 * float(self.settings.termination_unimix)
        initial_cap = float(self.settings.termination_max_probability_during_ramp)
        initial_raw = (
            self.settings.initial_termination_probability - term_floor
        ) / max(initial_cap - term_floor, 1.0e-8)
    ...
    def next_imagination_horizon(self) -> int:
        s = self.settings
        fraction = min(
            max(self._step_float() / float(s.imag_horizon_ramp_steps), 0.0),
            1.0,
        )
    ...
    def bounded_learned_termination_probability(
        ...
        """Smoothly map learned beta into the active execution interval.

        Unlike ``clamp(max=cap)``, this parameterisation retains a non-zero
        gradient everywhere except at ordinary sigmoid saturation.
        """
        raw = self.learned_termination_probability(feat, option, age)
        floor = 0.5 * float(self.settings.termination_unimix)
        cap = float(self.termination_probability_cap(step))
        if not floor < cap:
            raise RuntimeError("termination floor must be below execution cap")
        return floor + (cap - floor) * raw
