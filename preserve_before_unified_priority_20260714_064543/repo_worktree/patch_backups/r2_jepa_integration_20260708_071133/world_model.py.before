...
class FrozenJEPAWorldModel(nn.Module):
    ...
    Important mask convention used by this adapter:

    - encoded["entity_mask"] is still the raw current-observation visibility /
      observed-presence mask produced by online tokenization.
    - the packed JEPA state's entity_mask field is treated as the policy-facing
      belief exposure mask after obs_step/img_step.
    - slot_mask remains the structural slot-existence mask.

    This avoids using "not visible now" as "does not exist in belief" for
    anchored Exp33-style hidden-entity memory.
    """
    ...
    def _seen_mask_from_memory(self, memory: torch.Tensor) -> torch.Tensor | None:
        ...
        Exp33 anchored memory stores memory as:
            [recurrent | anchor latent | seen | hidden age]

        We intentionally read seen through the module's _split() method instead
        of hardcoding recurrent_dim offsets in this adapter.
        """
    ...
    def _belief_mask(
        ...
        """Build the mask that should be exposed to condition()/policy features.

        observed_or_presence_mask is allowed to mean:
        - current observation visibility/presence in obs_step/get_feat on real states
        - predicted rollout presence in img_step

        For anchored memory, hidden-but-seen entities should remain exposed even
        when they are not visible now. For legacy memory, keep old semantics.
        """
        dtype = memory.dtype
        ...
        # Use max rather than addition so soft predicted presence remains bounded.
    ...
    def encode_obs(self, obs: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        # Raw current-observation visibility / observed-presence mask.
    ...
    def obs_step(self, prev_z, prev_deter, prev_action, encoded, reset):
        memory, prev_exposure_mask, slot_mask, static = unpack_state(
            prev_deter, self.state_spec
        )
        ...
        with torch.no_grad():
            ...
            cur_exposure_mask = self._belief_mask(
                cur_visibility_mask, cur_slot, next_memory
            )

        return z, pack_state(
            next_memory.detach(), cur_exposure_mask.detach(), cur_slot, cur_static
        )
    ...
    def observe(self, encoded_sequence, action_sequence, initial_state, reset_sequence):
        ...
        if int(reset_sequence.shape[1]) != obs_len:
            raise ValueError(
                f"JEPA observe reset length {int(reset_sequence.shape[1])} "
                f"!= observation length {obs_len}"
            )
    ...
    def img_step(self, z, deter, action):
        ...
        with torch.no_grad():
            ...
            next_exposure_mask = self._belief_mask(
                next_presence_mask, slot_mask, next_memory
            )
