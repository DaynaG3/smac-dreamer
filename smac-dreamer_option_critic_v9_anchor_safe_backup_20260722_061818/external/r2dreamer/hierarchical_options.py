...
class HierarchicalOptionSettings:
    ...
    # A one-step minimum plus the preservation schedule below reproduces the
    # source Tactical Mixture's ability to reconsider its mode at every state.
    # Temporal commitment is introduced gradually rather than imposed abruptly.
    min_duration: int = 1
    max_duration: int = 8
    commitment_warmup_steps: int = 100_000
    commitment_full_steps: int = 600_000
    commitment_reselect_initial: float = 1.0
    commitment_reselect_final: float = 0.25

    initial_termination_probability: float = 0.10
    termination_warmup_steps: int = 350_000
    termination_full_steps: int = 800_000
    termination_max_probability_during_ramp: float = 0.30
    termination_max_probability_final: float = 0.30
    termination_cap_full_steps: int = 900_000
    ...
    termination_loss_scale: float = 0.02
    ...
    # Start exactly at the migrated source routing distribution. Exploration is
    # introduced slowly after the behaviour-preservation phase.
    ...
    manager_unimix_final: float = 0.005
    manager_unimix_decay_steps: int = 600_000
    slot_manager_unimix: float = 0.01
    slot_pair_unlock_initial_steps: int = 150_000
    slot_pair_unlock_interval_steps: int = 200_000
    slot_unlock_ramp_steps: int = 150_000
    slot_pg_ramp_steps: int = 200_000
    manager_pg_scale: float = 1.0
    manager_pg_warmup_steps: int = 100_000
    manager_pg_full_steps: int = 500_000
    ...
    # allowing four progressively unlocked slots inside each source group.
    ...
    # Keep the source JEPA representation fixed for this conservative 1M
    # stability phase. A later experiment may explicitly opt into adaptation.
    ...
    # Conservative variable option-imagination horizons. The active range moves
    # from 7-10 to 9-12; this extends credit assignment without jumping directly
    # to 15-step rollouts before the JEPA model is validated at that horizon.
    imag_horizon_initial_max: int = 10
    imag_horizon_final_max: int = 12
    imag_horizon_window: int = 4
    imag_horizon_ramp_steps: int = 600_000
...
class HierarchicalOptionsPolicy(nn.Module):
    ...
    ARCHITECTURE = "dreamer_option_critic_v6_progressive_8slot"
    SCHEMA_VERSION = 6
    ...
    def __init__(self, feature_dim: int, action_logit_dim: int, config: Any) -> None:
        ...
        # Factorized manager: source tactical group first, specialization slot
        # second. Only the anchor slot is active at migration, so the complete
        # option distribution exactly reproduces Tactical Mixture v1.2.
    ...
    def reset_parameters(self) -> None:
        ...
        # Slot logits start equal; the progressive gate makes only anchors 0/1
        # selectable at step zero.
    ...
    def slot_gate_by_slot(
        ...
        """Continuous availability gate for each within-group slot.

        Slot zero is the permanent source-backed anchor. Later slots are hard
        disabled before their unlock and then ramp from zero to one. Since slot
        deltas are separately gated, making a slot available cannot abruptly
        change the primitive policy.
        """
        x = self._step_float(step)
        gates = []
        for slot in range(self.slots_per_group):
            if slot == 0:
                gates.append(1.0)
                continue
            unlock = float(self.slot_unlock_step(slot))
            fraction = (x - unlock) / float(self.settings.slot_unlock_ramp_steps)
            gates.append(min(max(fraction, 0.0), 1.0))
        return torch.tensor(gates, dtype=torch.float32, device=self.training_step.device)
    ...
    def slot_pg_blend_for_option(
        ...
        """Task-PG maturity for the selected specialization slot."""
        x = self._step_float(step)
        slot = self.option_slot(option).to(dtype=torch.long)
        result = torch.ones_like(slot, dtype=torch.float32)
        for slot_index in range(1, self.slots_per_group):
            unlock = float(self.slot_unlock_step(slot_index))
            fraction = min(
                max((x - unlock) / float(self.settings.slot_pg_ramp_steps), 0.0),
                1.0,
            )
            result = torch.where(
                slot == slot_index,
                torch.full_like(result, fraction),
                result,
            )
        return result
    ...
    def slot_delta_scale_by_option(
        ...
        maturity = self.slot_pg_blend_for_option(option, step)
        # Anchor slots are exact source policies for the full run.
        return torch.where(
            slot == 0,
            torch.zeros_like(maturity),
            maturity * float(self.settings.slot_delta_scale_max),
        )
    ...
    def effective_termination_probability(
        ...
        learned_for_execution = self.bounded_learned_termination_probability(
            feat, option, age, step
        )
        global_blend = self.termination_blend(step)
        option_maturity = self.slot_pg_blend_for_option(option, step).to(
            device=learned_for_execution.device,
            dtype=learned_for_execution.dtype,
        )
        blend = global_blend * option_maturity
        fixed = torch.full_like(
            learned_for_execution, s.initial_termination_probability
        )
        mixed = fixed + blend * (learned_for_execution - fixed)
        # During migration preservation, reselect at every eligible state. This
        # exactly retains the source selector's per-state decision semantics and
        # then hands control to genuine call-and-return options gradually.
        reselect = self.commitment_reselect_probability(step)
        mixed = reselect + (1.0 - reselect) * mixed
