...
def hierarchical_auxiliary_loss(
    ...
    group_manager_loss = manager_policy_loss(
        group_manager_log_prob,
        group_manager_entropy,
        manager_adv,
        boundary,
    ...
    # Slot routing is learned only after the selected child has been unlocked
    # and matured. Group routing can therefore improve without immediately
    # sending probability into untrained duplicate slots.
    ...
    manager_loss = group_manager_loss + slot_manager_loss
    manager_entropy = group_manager_entropy + slot_manager_entropy
    ...
    *,
    ...
    with torch.no_grad():
        ...
        if target.num_options != 8:
            raise RuntimeError(
                f"v6 progressive migration requires eight slots, got {target.num_options}"
            )
        if target.settings.source_manager_group_count != 2:
            raise RuntimeError("v6 progressive migration requires two source groups")

        # Exact source group selector. Slot routing is a separate zero-initialized
        # head, and only anchor slots 0/1 are active at step zero.
        ...
        for row in range(target.num_options):
            target.slot_embedding.weight[row].copy_(source_embedding[row % 2])
    ...
    return {
        ...
        "migration_layout": "two_source_anchors_plus_six_progressive_child_slots",
