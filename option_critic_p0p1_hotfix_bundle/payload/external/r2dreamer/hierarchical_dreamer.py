...
def load_hierarchical_compatible_state(
    ...
    *,
    ...
    with torch.no_grad():
        ...
        # Expand each competent source mode into four exact copies. With
        # temperature 1.0, zero manager unimix, full migrated worker scale, and
        # one-step preservation reselection, grouped manager probabilities and
        # primitive action distributions exactly match Tactical Mixture v1.2 at
        # step zero. Independent option rows naturally break symmetry through
        # sampled task gradients; no task-agnostic perturbation is required.
        temperature = 1.0
        ...
        for row in range(target.num_options):
            source_index = row % 2
            target.option_embedding.weight[row].copy_(
    ...
    return {
        ...
        "migration_layout": "four_exact_copies_per_v1_2_mode",
        "trajectory_preservation": "per_state_reselection_then_gradual_commitment",
    }
