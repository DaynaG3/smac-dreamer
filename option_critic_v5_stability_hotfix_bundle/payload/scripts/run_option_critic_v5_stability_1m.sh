...
CONFIG="${CONFIG:-configs/r2_2100_jepa_option_critic_2_v5_stability_1m.yaml}"
...
AUDIT_LOG="$ROOT/option_critic_v5_stability_prelaunch_audit_$(date +%Y%m%d_%H%M%S).log"
echo "[AUDIT] running complete v5 stability prelaunch audit"
set +e
REPO="$REPO" PY="$PY" CONFIG="$CONFIG" CHECKPOINT="$CHECKPOINT_REAL" \
SOURCE_RUN_META="$META_REAL" bash scripts/static_audit_option_critic_v5_stability.sh \
  2>&1 | tee "$AUDIT_LOG"
...
RUN_DIR="${RUN_DIR:-$REPO/logs/r2dreamer/tactical_v12_option_critic_v5_stability_1m_$STAMP}"
if [[ -e "$RUN_DIR" ]]; then
...
)"
{
  ...
  echo "architecture=dreamer_option_critic_v5_stability"
  echo "worker_pg_ramp=20k_to_150k"
  echo "manager_pg_ramp=100k_to_500k"
  echo "commitment_reselection=1.0_to_0.25_from_100k_to_600k"
  echo "termination_ramp=350k_to_800k"
...
printf '%s\n' "$RUN_DIR" | tee "$ROOT/CURRENT_OPTION_CRITIC_V5_STABILITY_1M_RUN.txt"

echo "[START] Option-Critic v5 stability 1M run: $RUN_DIR"
