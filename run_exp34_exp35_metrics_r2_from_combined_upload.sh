...
# Run this script from combined-upload OR place it inside combined-upload.
#
# Expected layout:
#   combined-upload/
#     .venv/
...
#     smac-dreamer/
#
# Pipeline, continue-on-failure:
#   1) Train Exp34 JEPA for 5 epochs
#   2) Ordinary rollout/probe metric eval for Exp34
#   3) Hidden-belief metric eval for Exp34
#   4) Train Exp35 JEPA for 5 epochs
#   5) Ordinary rollout/probe metric eval for Exp35
#   6) Hidden-belief metric eval for Exp35
#   7) R2-Dreamer 2M using ORIGINAL Exp33 JEPA checkpoint
#
# It does NOT overwrite:
#   smac-dreamer/checkpoints/jepa/model.pt

# ---------------------------------------------------------------------
# Resolve combined-upload root and activate .venv
# ---------------------------------------------------------------------
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

if [ -d "$SCRIPT_DIR/smac-jepa-wm" ] && [ -d "$SCRIPT_DIR/smac-dreamer" ]; then
  COMBINED_ROOT="${COMBINED_ROOT:-$SCRIPT_DIR}"
elif [ -d "$SCRIPT_DIR/../smac-jepa-wm" ] && [ -d "$SCRIPT_DIR/../smac-dreamer" ]; then
  COMBINED_ROOT="${COMBINED_ROOT:-$(cd "$SCRIPT_DIR/.." && pwd)}"
else
  COMBINED_ROOT="${COMBINED_ROOT:-$HOME/workspace/dreamer/combined-upload}"
fi

JEPA_ROOT="${JEPA_ROOT:-$COMBINED_ROOT/smac-jepa-wm}"
DREAMER_ROOT="${DREAMER_ROOT:-$COMBINED_ROOT/smac-dreamer}"
VENV_DIR="${VENV_DIR:-$COMBINED_ROOT/.venv}"
...
if [ -f "$VENV_DIR/bin/activate" ]; then
  ...
  echo "[warn] no venv found at $VENV_DIR; using current Python environment"
fi

export PYTHONPATH="$JEPA_ROOT:$DREAMER_ROOT/smac-dreamer/src:${PYTHONPATH:-}"

STAMP="${STAMP:-$(date +%Y%m%d_%H%M%S)}"
PIPELINE_LOG_DIR="${PIPELINE_LOG_DIR:-$COMBINED_ROOT/overnight_logs/exp34_exp35_true_metrics_r2_${STAMP}}"
...
abs_path_if_exists() {
  ...
  if [ -f "$JEPA_ROOT/$p" ]; then
    cd "$(dirname "$JEPA_ROOT/$p")" && printf "%s/%s\n" "$(pwd)" "$(basename "$JEPA_ROOT/$p")"
    return 0
  fi
  if [ -f "$COMBINED_ROOT/$p" ]; then
    cd "$(dirname "$COMBINED_ROOT/$p")" && printf "%s/%s\n" "$(pwd)" "$(basename "$COMBINED_ROOT/$p")"
    return 0
  fi
  return 1
}
...
resolve_eval_files() {
  ...
  if [ -n "$ORDINARY_EVAL" ]; then
    ORDINARY_EVAL="$(abs_path_if_exists "$ORDINARY_EVAL" || true)"
  else
  ...
  if [ -n "$HIDDEN_EVAL" ]; then
    HIDDEN_EVAL="$(abs_path_if_exists "$HIDDEN_EVAL" || true)"
  else
