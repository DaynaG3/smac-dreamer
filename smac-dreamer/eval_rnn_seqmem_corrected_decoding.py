...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate RNN seqmem checkpoints with padding-aware decoded-state, action-sensitivity, "
...
def evaluate_one_checkpoint(
    ...
    autonomous_mae = corrected_metrics.get(
        "corrected_autonomous_mask_dynamic_mae"
    )
    ...
    if native_mae is not None and shuffled_mae is not None:
        row[
            "eval_corrected_wrong_action_dynamic_mae_penalty"
        ] = shuffled_mae - native_mae

    if native_mae is not None and zero_memory_mae is not None:
        row[
            "eval_corrected_memory_dynamic_mae_gain"
        ] = zero_memory_mae - native_mae

    if native_mae is not None and autonomous_mae is not None:
        row[
            "eval_corrected_oracle_mask_dynamic_mae_advantage"
        ] = autonomous_mae - native_mae
