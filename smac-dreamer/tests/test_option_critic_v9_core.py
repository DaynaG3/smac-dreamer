...
def test_forecast_wrapper_propagates_root_and_completes_all_fake_stages(tmp_path):
    import os
    import subprocess
    import zipfile

    root = tmp_path / "workspace"
    jepa = root / "smac-jepa-wm"
    venv = root / ".venv"
    jepa.mkdir(parents=True)
    (venv / "bin").mkdir(parents=True)
    (venv / "bin/activate").write_text("true\n", encoding="utf-8")
    bundle_dir = tmp_path / "bundle"
    bundle_dir.mkdir()
    installer = bundle_dir / "install_exp45_pow2_direct.sh"
    installer.write_text(
        r'''#!/usr/bin/env bash
set -euo pipefail
printf '%s\n' "$ROOT" > "$ROOT/seen_exp45_installer_root.txt"
J="$ROOT/smac-jepa-wm"
mkdir -p "$J/smac_jepa" "$J/scripts" "$J/splits" \
  "$J/runs/rnn_seqmem_exp40_event_balanced_5ep_20260709_083104"
touch "$J/smac_jepa/train_jepa_exp45_pow2_direct.py"
touch "$J/splits/r2_general_2100.json"
touch "$J/runs/rnn_seqmem_exp40_event_balanced_5ep_20260709_083104/checkpoint.pt"
cat > "$J/scripts/static_audit_exp45_pow2.sh" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
cat > "$J/scripts/run_exp45_pow2_direct_train.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
mkdir -p "$OUT_DIR"
printf x > "$OUT_DIR/checkpoint.pt"
EOF
for s in eval_exp45_pow2_ordinary.sh eval_exp45_pow2_hidden.sh; do
cat > "$J/scripts/$s" <<'EOF'
#!/usr/bin/env bash
exit 0
EOF
...
''',
    ...
    zip_path = root / "exp45_pow2_direct_from_exp40_bundle.zip"
    root.mkdir(parents=True, exist_ok=True)
    with zipfile.ZipFile(zip_path, "w") as zf:
        zf.write(installer, "nested/install_exp45_pow2_direct.sh")

    script = ROOT / "scripts/run_exp45_full_train_eval_resilient.sh"
    pipe_dir = root / "forecast-test"
    env = os.environ.copy()
    env.update(
        ROOT=str(root),
        JEPA_ROOT=str(jepa),
    ...
    assert (root / "seen_exp45_installer_root.txt").read_text().strip() == str(root)
    status = (pipe_dir / "status.tsv").read_text(encoding="utf-8")
    for stage in ("install", "static_audit", "training", "ordinary_eval", "hidden_eval"):
        assert f"{stage}\tCOMPLETED" in status
