...
printf '%s\n' "$PIPE_DIR" > "$ROOT/CURRENT_EXP45_FORECAST_PIPELINE.txt"
...
# Install only if the trainer is absent.
if [[ ! -f "$JEPA_ROOT/smac_jepa/train_jepa_exp45_pow2_direct.py" ]]; then
  if [[ -f "$BUNDLE_ZIP" ]]; then
    INSTALL_TMP="$PIPE_DIR/install_bundle"; rm -rf "$INSTALL_TMP"; mkdir -p "$INSTALL_TMP"
    if unzip -q "$BUNDLE_ZIP" -d "$INSTALL_TMP"; then
      INSTALLER="$(find "$INSTALL_TMP" -type f -name install_exp45_pow2_direct.sh -print -quit)"
      if [[ -n "$INSTALLER" ]]; then
        chmod +x "$INSTALLER"
        # The user-provided installer defaults ROOT from $HOME. The training
        # container runs as root while the actual workspace is under
        # /home/jovyan, so ROOT must be propagated explicitly.
        run_stage install "$LOG_DIR/install.log" env ROOT="$ROOT" bash "$INSTALLER" || true
      else
        record install FAILED 127 "installer absent in $BUNDLE_ZIP"; FAILURES=$((FAILURES+1))
      fi
    else
      record install FAILED 2 "could not extract $BUNDLE_ZIP"; FAILURES=$((FAILURES+1))
    fi
  else
    record install FAILED 2 "bundle missing: $BUNDLE_ZIP"; FAILURES=$((FAILURES+1))
  fi
else
  record install SKIPPED 0 "trainer already installed"
fi

# Installation is considered usable only when the exact trainer and all four
# stage launchers exist under the requested JEPA_ROOT. This prevents a successful
# installer exit in the wrong $HOME-derived workspace from being mistaken for a
# valid setup.
for required in \
  ...
  if [[ ! -f "$JEPA_ROOT/$required" ]]; then
    record install_verify FAILED 2 "missing after install: $JEPA_ROOT/$required"
    FAILURES=$((FAILURES + 1))
  fi
done
