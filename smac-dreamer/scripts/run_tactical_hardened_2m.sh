...
[[ -s "$CONFIG_PATH" ]] || {
  echo "[FAIL] hardened config missing: $CONFIG_PATH" >&2
  exit 1
}
[[ -s "$CHECKPOINT" ]] || {
...
if [[ "$(basename "$CHECKPOINT")" != "best_val_macro_winrate.pt" ]]; then
  echo "[FAIL] tactical hardening must start from best_val_macro_winrate.pt" >&2
  exit 1
fi
[[ -s "$SOURCE_RUN_META" ]] || {
...
bash "$REPO/scripts/static_audit_tactical_hardening.sh"
...
RUN_DIR="${RUN_DIR:-$REPO/logs/r2dreamer/adaptive_best_tactical_hardened_2m_$STAMP}"
if [[ -e "$RUN_DIR" ]]; then
  if [[ ! -d "$RUN_DIR" ]]; then
...
printf '%s\n' "$RUN_DIR" | tee "$ROOT/CURRENT_TACTICAL_HARDENED_RUN.txt"

echo "[START] hardened tactical run: $RUN_DIR"
