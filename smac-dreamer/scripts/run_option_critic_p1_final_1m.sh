...
CONFIG="${CONFIG:-configs/r2_2100_jepa_option_critic_8_v4_p1_final_1m.yaml}"
...
AUDIT_LOG="$ROOT/option_critic_p1_final_prelaunch_audit_$(date +%Y%m%d_%H%M%S).log"
echo "[AUDIT] running complete P1-final prelaunch audit"
set +e
REPO="$REPO" PY="$PY" CONFIG="$CONFIG" CHECKPOINT="$CHECKPOINT_REAL" \
SOURCE_RUN_META="$META_REAL" bash scripts/static_audit_option_critic_p1_final.sh \
  2>&1 | tee "$AUDIT_LOG"
...
RUN_DIR="${RUN_DIR:-$REPO/logs/r2dreamer/tactical_v12_option_critic_v4_p1_final_1m_$STAMP}"
if [[ -e "$RUN_DIR" ]]; then
...
)"
{
  ...
  echo "architecture=dreamer_option_critic_v4_p1_final"
  echo "worker_pg_ramp=20k_to_100k"
  echo "manager_pg_and_commitment_ramp=100k_to_250k"
  echo "termination_ramp=250k_to_400k"
  echo "termination_cap_relaxation=400k_to_600k_smooth_min"
  echo "source_manager_group_trust_region=enabled"
  echo "world_model_gradient_scale=0"
  echo "hierarchy_imagination_horizon=variable_7_to_15"
  echo "validation=startup_and_every_200k"
  echo "task_agnostic_diversity_losses=disabled"
} | tee "$RUN_DIR/SOURCE_LINEAGE.txt"

printf '%s\n' "$RUN_DIR" | tee "$ROOT/CURRENT_OPTION_CRITIC_V4_P1_1M_RUN.txt"

echo "[START] Option-Critic P1-final 1M run: $RUN_DIR"
