...
REPLACED = (
    ...
    # Shared with earlier post-v4 packages. It must be backed up and
    # replaced transactionally rather than treated as a new v9 path.
...
def validate_source(repo: pathlib.Path) -> None:
    ...
    def validate_payload(relative: str) -> None:
        ...
        if payload_path.suffix == ".py":
            try:
                ...
                die(f"Python payload failed to parse: {relative}: {exc}")

        elif payload_path.suffix == ".sh":
