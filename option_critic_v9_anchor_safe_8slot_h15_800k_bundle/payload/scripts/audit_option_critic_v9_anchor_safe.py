...
source_meta_path = args.source_run_meta.resolve()

for path, label in (
    (repo, "repo"),
    (config_path, "config"),
    (checkpoint_path, "checkpoint"),
    (source_meta_path, "source run metadata"),
):
    if not path.exists():
...
state = checkpoint.get("agent_state_dict") or {}
...
if float(checkpoint.get("val_macro_win_rate", -1.0)) < 0.3749:
    fail("source checkpoint is not the expected best-validation checkpoint")
...
installed = {
    ...
    "master_launcher": repo / "scripts/run_forecast_first_then_option_critic_v9_800k.sh",
}
...
for token in (
    ...
    "CONTINUE_ON_FAILURE",
    "SKIPPED_ACTIVE_FORECAST_PROCESS",
):
