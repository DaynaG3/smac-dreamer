...
class HierarchicalOptionSettings:
    ...
    manager_unimix_decay_steps: int = 250_000
    manager_pg_scale: float = 1.0
    manager_pg_warmup_steps: int = 100_000
    manager_pg_full_steps: int = 250_000
    ...
    worker_pg_full_steps: int = 100_000
    ...
    # Preserve the source Tactical Mixture's two-way routing distribution. The
    # eight option slots are grouped by source mode using option_id % 2, so
    # duplicate slots may specialize without silently switching tactical group.
    source_manager_group_count: int = 2
    manager_group_kl_target: float = 0.005
    manager_group_kl_tail_target: float = 0.02
    manager_group_kl_tail_fraction: float = 0.10
    manager_group_kl_tail_relative_scale: float = 1.0
    manager_group_kl_scale: float = 0.25
    manager_group_preservation_confidence: float = 0.80
    manager_group_preservation_margin: float = 0.05
    manager_group_preservation_scale: float = 0.25
    ...
    imag_horizon_ramp_steps: int = 400_000
