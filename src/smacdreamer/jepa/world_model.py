...
class FrozenJEPAWorldModel(nn.Module):
    ...
    def initial(self, batch_size: int, *, device=None, dtype=torch.float32):
        ...
        memory = self.memory_module.initial_memory(batch_size, self.entities, device=device, dtype=dtype)
    ...
    def obs_step(self, prev_z, prev_deter, prev_action, encoded, reset):
        memory, entity_mask, slot_mask, static = unpack_state(prev_deter, self.state_spec)
        ...
        z = encoded["z"].detach()
        cur_mask = encoded["entity_mask"].detach()
        cur_slot = encoded["slot_mask"].detach()
        cur_static = encoded["static_condition"].detach()
        action, action_mask = self.action_adapter.flat_to_jepa(prev_action, cur_slot[:, : self.max_agents])
        with torch.no_grad():
            if getattr(self.memory_module, "uses_action", False):
                next_memory = self.memory_module.update(prev_z.detach(), memory, entity_mask, action=action, action_mask=action_mask)
            else:
                next_memory = self.memory_module.update(prev_z.detach(), memory, entity_mask)
    ...
    def img_step(self, z, deter, action):
        memory, entity_mask, slot_mask, static = unpack_state(deter, self.state_spec)
        action_jepa, action_mask = self.action_adapter.flat_to_jepa(action, slot_mask[:, : self.max_agents])
        with torch.no_grad():
            conditioned = self.memory_module.condition(z, memory, entity_mask)
            ...
            next_mask = (torch.sigmoid(logits) >= self.presence_threshold).to(dtype=z.dtype) * slot_mask
            pred = pred * next_mask.unsqueeze(-1)
            if getattr(self.memory_module, "uses_action", False):
                next_memory = self.memory_module.update(pred, memory, next_mask, action=action_jepa, action_mask=action_mask)
            else:
                next_memory = self.memory_module.update(pred, memory, next_mask)
        if not torch.isfinite(pred).all():
            raise FloatingPointError("non-finite JEPA predicted latent")
        return pred.detach(), pack_state(next_memory.detach(), next_mask.detach(), slot_mask, static)
    ...
    def get_feat(self, z, deter):
        memory, entity_mask, _, static = unpack_state(deter.reshape(-1, deter.shape[-1]), self.state_spec)
        z_flat = z.reshape(memory.shape[0], self.entities, self.latent_dim)
        with torch.no_grad():
            conditioned = self.memory_module.condition(z_flat, memory, entity_mask)
        feat = self.feature_adapter(conditioned.detach(), memory.detach(), entity_mask.detach(), static.detach())
