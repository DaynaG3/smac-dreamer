...
#   1) audited 1M-step Option-Critic v6 progressive 8-slot run
...
SESSION_NAME="${SESSION_NAME:-ocv6_8slot_1m_then_exp45_$PIPE_STAMP}"
PIPE_DIR="${MASTER_PIPE_DIR:-$ROOT/option_critic_v6_8slot_1m_then_exp45_$PIPE_STAMP}"
...
if [[ "$AUTO_TMUX" == "1" && -z "${TMUX:-}" && "${PIPELINE_INSIDE_TMUX:-0}" != "1" ]]; then
  ...
  printf '%s\n' "$PIPE_DIR" > "$ROOT/CURRENT_OPTION_CRITIC_V6_AND_EXP45_PIPELINE.txt"
...
if [[ -z "$TACTICAL_V12_RUN" ]]; then
  ...
  if run_stage "rl_1m" "$LOG_DIR/rl_1m.log" \
      env \
        ...
        FINAL_STEP=1000000 \
        "$REPO/scripts/run_option_critic_v6_progressive_1m.sh"; then
    RL_STATUS=0
  else
    RL_STATUS=$?
  fi
...
================================================================
Option-Critic v6 progressive 8-slot 1M -> Exp45 forecast-JEPA pipeline finished
================================================================
Pipeline:          $PIPE_DIR
Status:            $STATUS_FILE
Logs:              $LOG_DIR
RL pointer:         $ROOT/CURRENT_OPTION_CRITIC_V6_PROGRESSIVE_8SLOT_1M_RUN.txt
