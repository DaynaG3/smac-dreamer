...
Version 1.1 keeps the original primitive actor and action-mask contract intact,
...
class TacticalMixturePolicy(nn.Module):
    ...
    def reset_parameters(self) -> None:
        # Uniform initial selector. The tiny output-layer perturbation below is
        # enough to break tactic symmetry without discarding inherited policy.
    ...
    def residual_logits(self, feat: torch.Tensor, tactic: torch.Tensor) -> torch.Tensor:
        ...
        # Do not call ``tactic.min().item()`` or ``max().item()`` here: this
        # path runs at every real and imagined action and a Python scalar check
        # would force a GPU synchronization. ``nn.Embedding`` already raises on
        # an out-of-range index, while all production tactics come from the
        # selector's Categorical distribution.
        emb = self.embedding(tactic)
        raw = self.residual(torch.cat([feat.float(), emb], dim=-1))
    ...
    def eval_combined_logits(
        ...
        uniform_confidence = 1.0 / float(self.num_tactics)
        denominator = max(
            self.settings.eval_confidence_threshold - uniform_confidence,
            1.0e-8,
        )
        gate = (
            (confidence - uniform_confidence) / denominator
        ).clamp(0.0, 1.0)
    ...
    def all_residual_logits(self, feat: torch.Tensor) -> torch.Tensor:
        """Return tactic residuals with shape (..., K, action_dim)."""
        ...
        return self.residual_logits(feat_all, tactic)
    ...
    def usage_statistics(
        ...
        collapse_loss = max_excess.square() + effective_shortfall.square()
        ...
        return {
            ...
            "mutual_information_normalized": mutual_information / log_k,
    ...
    def assert_legacy_equivalence_ready(self) -> None:
        """Compatibility name retained; now validates a bounded warm start.

        Exact zero residuals create a zero-gradient diversity fixed point. The
        hardened version permits only the configured tiny output perturbation.
        """
        if torch.count_nonzero(self.selector[-1].weight).item() != 0:
            raise RuntimeError("selector final weight is not zero-initialized")
        ...
        if not torch.isfinite(self.residual[-1].weight).all():
            raise RuntimeError("residual final weight contains non-finite values")
        observed = float(
            self.residual[-1].weight.detach().std(unbiased=False)
        )
        expected = self.settings.symmetry_break_std / math.sqrt(
            float(self.settings.hidden_dim)
        )
        # Wide enough for finite-sample variation, tight enough to catch an
        # accidentally trained or grossly initialized residual head.
        upper = max(1.0e-10, expected * 4.0)
        if observed > upper:
            raise RuntimeError(
                f"residual symmetry-break std {observed:.6g} exceeds safe "
