...
AUDIT_LOG="$ROOT/option_critic_prelaunch_audit_$(date +%Y%m%d_%H%M%S).log"

echo "[AUDIT] running complete prelaunch audit"
...
if (( AUDIT_STATUS != 0 )); then
  ...
  tail -n 120 "$AUDIT_LOG" >&2 || true
  exit "$AUDIT_STATUS"
fi

echo "[OK] complete prelaunch audit passed"
