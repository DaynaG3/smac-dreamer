    ...
    with tempfile.TemporaryDirectory(prefix="v9_three_stage_") as td:
        ...
        try:
            for rel in INTRODUCED:
                ...
                # The staging directory may be on a different filesystem
                # from the repository. Copy to a temporary sibling first,
                # then atomically rename inside the destination filesystem.
                tmp_dst = dst.with_name(
                    f".{dst.name}.v9_three_stage_{os.getpid()}.tmp"
                )
                try:
                    shutil.copy2(staged[rel], tmp_dst)
