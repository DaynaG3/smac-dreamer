...
EXPECTED_SHA = "74875c693150d4cd21be27201e332cb0d8d4f6648c10701761154dcd6588d99e"
...
def main() -> None:
    ...
    p.add_argument("--expected-sha256", default=EXPECTED_SHA)
    args = p.parse_args()
    ...
    state = ckpt.get("agent_state_dict") or {}
    assert meta.get("architecture") == "tactical_mixture_v1_2"
    assert int(meta.get("num_tactics", -1)) == 2
    assert float(ckpt.get("val_macro_win_rate", -1.0)) >= 0.3749
