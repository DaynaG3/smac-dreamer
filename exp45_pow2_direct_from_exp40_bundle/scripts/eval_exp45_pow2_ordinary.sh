...
find_ordinary_eval() {
  ...
  for path in \
    ...
    tools/eval_rnn_seqmem_dreamer_probe_r2aware.py; do
...
python tools/make_exp40_eval_checkpoint.py \
  --checkpoint "$CHECKPOINT" \
  --out "$SANITIZED"

# Trusted apples-to-apples Exp40 recursive evaluation at H=5.
R2_AWARE_BASE_EVAL="$ORDINARY_EVAL_PATH" \
python tools/eval_rnn_seqmem_dreamer_probe_r2aware_anchored.py \
