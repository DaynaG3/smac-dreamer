...
export SMAC_JEPA_EXP35_SIMPLE_LOSS=1
export SMAC_JEPA_PRESENCE_NEG_CLASS_WEIGHT=3.0
#!/usr/bin/env bash
set -Eeuo pipefail
...
OUT_DIR="${OUT_DIR:-runs/rnn_seqmem_exp35_two_mask_simple_loss}"
EPOCHS="${EPOCHS:-7}"
...
WANDB_NAME="${WANDB_NAME:-exp35-two-mask-simple-loss}"
...
from smac_jepa import train_jepa_exp31_exp35 as trainer
...
python -m smac_jepa.train_jepa_exp35_dreamer \
  --manifest "$MANIFEST" \
  --split train \
  --out-dir "$OUT_DIR" \
  --model-size default \
