...
OUT_DIR="${OUT_DIR:-runs/rnn_seqmem_exp41_hidden_change_gate}"
EPOCHS="${EPOCHS:-5}"
...
WANDB_NAME="${WANDB_NAME:-exp41-hidden-change-gate}"
...
python -m smac_jepa.train_jepa_exp41_dreamer \
  ...
  --delta-loss-weight 0.05 --event-dynamics-weight 1.00 --event-dynamics-threshold 0.01 \
  ...
  --hidden-change-residual-weight 0.06 --hidden-change-gate-weight 0.02 --hidden-change-gate-temperature 0.01 \
  --inverse-dynamics-weight 0.01 --inverse-dynamics-hidden-dim 256 \
  --memory-barlow-scale 0.0 --no-event-balanced-sampling --no-residual-state-decoder --no-direct-action-fusion \
...
python scripts/validate_exp33_dreamer_checkpoint.py "$OUT_DIR/checkpoint.pt" --jepa-root . --dreamer-root ../smac-dreamer
