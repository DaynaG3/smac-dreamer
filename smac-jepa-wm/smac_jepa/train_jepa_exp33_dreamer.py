...
``train_jepa_exp31_exp33`` trainer. It preserves the complete Exp33 memory
...
def _require_full_base_trainer() -> None:
    ...
    if missing:
        raise SystemExit(
            ...
            + ", ".join(missing)
            + ". Copy the latest working smac_jepa/train_jepa_exp31_exp33.py "
...
def _patch_for_exp33_dreamer() -> None:
    ...
    def parse_args():
        ...
        if float(args.inverse_dynamics_weight) != 0.0:
            errors.append("inverse dynamics is not part of the retained Exp33 recipe")
        ...
        if bool(args.event_balanced_sampling):
            errors.append("event-balanced sampling is not part of Exp33")
